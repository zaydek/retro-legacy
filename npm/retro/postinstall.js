...
async function run() {
	...
	if (!binary) {
		console.error(
			`[retro]: Your platform is not yet supported; platformKey=${JSON.stringify(platformKey)}. ` +
	...
	try {
		...
		throw new Error(`[retro]: An unexpected error occurred; err.message=${JSON.stringify(err.message)}.`)
	}
}
